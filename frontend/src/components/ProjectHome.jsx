...
export function ProjectHome(props) {
    ...
    const fetchTodo = useCallback(async () => {
        ...
        try {
            ...
            const response = await axios.get(`${backendUrl}/project/todo/todos`, {
                params: {
    ...
    const addTodo = useCallback(async () => {
        ...
        try {
            ...
            const response = await axios.post(`${backendUrl}/project/todo`, {
                title: toDoTitle,
    ...
    const updateTodoStatus = async (id, status) => {
        try {
            let safeToken = localStorage.getItem('token');
            const response = await axios.put(`${backendUrl}/project/todo/updateStatus`, {
                id: id,
                status: status
            }, {
                headers: {
            ...
            <div className="w-full px-2 sm:px-16 py-8 flex flex-col sm:flex-row justify-between items-center">
                ...
                <div className="flex justify-between">
                    <div className="inline-flex sm:items-center p-2 rounded-lg cursor-pointer dark:text-gray-800" onClick={() => {
                        ...
                        <span
                            className={`px-4 py-2 rounded-l-lg ${props.toggleState ? 'bg-blue-marguerite-300 peer-checked:bg-gray-300' : 'bg-gray-300 peer-checked:bg-blue-marguerite-300'
                                } transition-colors duration-300`}
                        >
                            Tasks
                        </span>
                        <span
                            className={`px-4 py-2 rounded-r-lg ${props.toggleState ? 'bg-gray-300 peer-checked:bg-blue-marguerite-300' : 'bg-blue-marguerite-300 peer-checked:bg-gray-300'
                                } transition-colors duration-300`}
                        >
                            Dashboard
                        </span>
